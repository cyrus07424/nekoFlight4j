...
public class Jflight extends Applet3D implements Runnable {
    ...
    private static final String[] UI_MENU_LABELS = {
            "Pitch Ladder", "Reticle", "Lock Box", "Enemy Arrows", "Heading Tape", "Speed Tape", "Altitude Tape", "TGT Panel", "ACC / G", "AAM Ammo", "Gun Heat", "Header", "Mode Banner", "Footer",
    };
    ...
    protected boolean uiPitchLadderVisible = true;
    ...
    protected boolean uiHeadingVisible = true;
    ...
    public void resetStagePreserveUi() {
        ...
        boolean savedUiPitchLadderVisible = uiPitchLadderVisible;
        ...
        boolean savedUiHeadingVisible = uiHeadingVisible;
        ...
        uiPitchLadderVisible = savedUiPitchLadderVisible;
        ...
        uiHeadingVisible = savedUiHeadingVisible;
    ...
    private void toggleUiMenuValue(int index) {
        switch (index) {
            case 0:
                uiPitchLadderVisible = !uiPitchLadderVisible;
                break;
            case 1:
            ...
            case 3:
                ...
                break;
            case 4:
                uiHeadingVisible = !uiHeadingVisible;
                break;
            case 5:
    ...
    public void draw() {
        synchronized (renderLock) {
            ...
            if (uiPitchLadderVisible)
                drawPitchLadderHud(plane[0]);
    ...
    private int normalizeHeadingDegrees(int headingDeg) {
        int normalized = headingDeg % 360;
        if (normalized < 0)
            normalized += 360;
        return normalized;
    }

    private String headingLabel(int headingDeg) {
        switch (normalizeHeadingDegrees(headingDeg)) {
            case 0:
                return "N";
            case 90:
                return "E";
            ...
            case 270:
                return "W";
            default:
                return String.format("%03d", normalizeHeadingDegrees(headingDeg));
        }
    }

    private void drawHudHeadingTape(int centerX, int topY, int headingDeg, int stepDeg, int majorStepDeg, int rangeDeg,
                                    Color color) {
        if (bGraphics == null)
            return;

        double scale = hudScale();
        double tapeScale = scale * 0.25;
        int tapeW = (int) Math.round(256 * tapeScale);
        int tapeH = (int) Math.round(18 * tapeScale);
        int leftX = centerX - tapeW / 2;
        int rightX = leftX + tapeW;
        int tickBottomY = topY + tapeH - 2;
        int labelBaseY = topY + bGraphics.getFontMetrics().getAscent();
        int centerMarkH = (int) Math.max(2, Math.round(4 * tapeScale));
        double pixelsPerStep = (stepDeg > 0 && rangeDeg > 0)
                ? (double) tapeW * stepDeg / (2.0 * rangeDeg)
                : 1.0;
        int normalizedHeading = normalizeHeadingDegrees(headingDeg);

        bGraphics.setColor(color);

        int firstTick = (int) Math.floor((double) (normalizedHeading - rangeDeg) / stepDeg) * stepDeg;
        int lastTick = (int) Math.ceil((double) (normalizedHeading + rangeDeg) / stepDeg) * stepDeg;
        for (int tick = firstTick; tick <= lastTick; tick += stepDeg) {
            int x = centerX + (int) Math.round((double) (tick - normalizedHeading) / stepDeg * pixelsPerStep);
            if (x < leftX || x > rightX)
                continue;

            int wrappedTick = normalizeHeadingDegrees(tick);
            boolean major = wrappedTick % majorStepDeg == 0;
            int tickTopY = major ? labelBaseY + 2 : tickBottomY - (int) Math.max(2, Math.round(4 * tapeScale));
            bGraphics.drawLine(x, tickTopY, x, tickBottomY);

            if (major) {
                String tickLabel = headingLabel(wrappedTick);
                FontMetrics metrics = bGraphics.getFontMetrics();
                int textX = x - metrics.stringWidth(tickLabel) / 2;
                if (textX >= leftX && textX + metrics.stringWidth(tickLabel) <= rightX)
                    bGraphics.drawString(tickLabel, textX, labelBaseY);
            }
        }

        bGraphics.drawLine(centerX, topY + tapeH, centerX, topY + tapeH + centerMarkH);
    }
    ...
    private void drawPitchLadderHud(Plane player) {
        int cx = sCenterX;
        int cy = sCenterY;
    ...
    private void drawHud() {
        ...
        Plane player = plane[0];
        double scale = hudScale();
        int headingDeg = normalizeHeadingDegrees((int) Math.round(Math.toDegrees(player.aVel.z)));

        Stroke oldStroke = bGraphics.getStroke();
        bGraphics.setStroke(new BasicStroke((float) Math.max(1.0, scale * 0.35)));

        if (chromeVisible && uiModeBannerVisible)
            drawModeBanner();

        if (chromeVisible) {
            int headingTopY = (int) Math.round(24 * scale);
            if (uiHeadingVisible)
                drawHudHeadingTape(sCenterX, headingTopY, headingDeg, 5, 30, 60, Colors.GREEN_YELLOW);
        }
